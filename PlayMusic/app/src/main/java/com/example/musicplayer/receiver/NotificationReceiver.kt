...
class NotificationReceiver : BroadcastReceiver() {
    ...
    private fun nextSong(context: Context) {
        ...
        if (!PlayMusicActivity.isShuffle) {
            when (ApplicationClass.type) {
                "chart-realtime" -> if (index < PlayMusicActivity.musicList.size - 1) index++ else index = 0
                "search" -> if (index < PlayMusicActivity.songSearchList.size - 1) index++ else index = 0
                "offline" -> if (index < PlayMusicActivity.songLocalList.size - 1) index++ else index = 0
                "favourite" -> if (index < PlayMusicActivity.songFavouriteList.size - 1) index++ else index = 0
            }
        } else {
            when (ApplicationClass.type) {
                "chart-realtime" -> index = Random.nextInt(0, PlayMusicActivity.musicList.size)
