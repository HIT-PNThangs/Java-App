    ...
    override fun onCompletion(p0: MediaPlayer?) {
        if (!repeatOne) {
            if (isShuffle) {//checkType
                ...
                when (ApplicationClass.type) {
                    "chart-realtime" -> {
                        if (indexSong < ApplicationClass.listChartRealtime.size - 1) indexSong++ else indexSong = 0
                    ...
                    "search" -> {
                        if (indexSong < ApplicationClass.listSongSearch.size - 1) indexSong++ else indexSong = 0
                    ...
                    "offline" -> {
                        if (indexSong < ApplicationClass.listSongLocal.size - 1) indexSong ++ else indexSong = 0
